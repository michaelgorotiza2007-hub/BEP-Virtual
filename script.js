...
window.registrarUsuario = async () => {
    ...
    if(isAdminCheck) {
        ...
        else if(adminCode === "CAJA2025") rol = 'cajero'; // Añadido soporte cajero
...
window.cerrarSesion = () => {
    usuario = null; localStorage.removeItem('bep_active_user'); location.reload();
};
...
function actualizarUI() {
    ...
    const approved = usuario.solicitudes.find(s => s.estado === 'aprobado');
    ...
    if(pending) {
        ...
        stBox.innerText = 'Tienes un préstamo activo.';
        stBox.style.color = '#2ecc71';
        amortBox.classList.remove('hidden');
        generarHTMLAmortizacion(approved, 'tabla-amortizacion-user');
    } else {
        stBox.innerText = 'No hay solicitudes activas.';
    ...
    if (approved) {
        const deudaTotal = approved.saldoPendiente !== undefined ? approved.saldoPendiente : approved.totalPagar;
...
window.operar = async (tipo) => {
    ...
    if(tipo === 'deposito-propio') {
        ...
        if(monto > 0 && benef) {
            ...
            await guardarUsuario(); mostrarToast('Cheque depositado', 'success');
        }
...
window.solicitarPrestamo = async () => {
    ...
    await guardarUsuario();
    mostrarToast(`Solicitud enviada. Total: $${totalPagar.toFixed(2)}`, 'success');
...
window.cargarDashboardAdmin = async () => {
    ...
    let totalCapital = 0; let usuariosConSaldo = 0; let usuariosSinSaldo = 0;
    ...
    document.getElementById('estado-reserva').innerText = totalCapital > 5000 ? "Saludable" : "Baja";
...
function renderCharts(conSaldo, sinSaldo, capital) {
    ...
    chartInstance = new Chart(ctx1, { type: 'doughnut', data: { labels: ['Con Saldo', 'Sin Saldo'], datasets: [{ data: [conSaldo, sinSaldo], backgroundColor: ['#2ecc71', '#e74c3c'], borderWidth: 0 }] } });
    barInstance = new Chart(ctx2, { type: 'bar', data: { labels: ['Capital Total', 'Reserva Objetivo'], datasets: [{ label: 'USD', data: [capital, 10000], backgroundColor: ['#e6b333', '#3498db'] }] }, options: { scales: { y: { beginAtZero: true } } } });
}

// CORRECCIÓN ADMIN: LISTAS SEPARADAS Y BOTÓN OJO
        ...
        if (u.rol === 'cliente') {
            tClientes.innerHTML += `<tr>
                ...
                <td><div style="display:flex;gap:5px;">
                    <button onclick="verDetalleCliente('${u.username}')" class="btn-blue" title="Ver"><i class="fas fa-eye"></i></button>
                    <button onclick="eliminarUsuario('${u.username}')" class="btn-red" title="Borrar"><i class="fas fa-trash"></i></button>
                </div></td>
            </tr>`;
        } 
        // TABLA TRABAJADORES
...
window.verDetalleCliente = async (usernameId) => {
    try {
        ...
        const amortTable = document.getElementById('detalle-amortizacion-container');
        
        if (prestamo) {
            const deuda = prestamo.saldoPendiente !== undefined ? prestamo.saldoPendiente : prestamo.totalPagar;
            infoDiv.innerHTML = `<div class="stat-card" style="border:1px solid #2ecc71;"><h4 class="text-green">Préstamo Activo</h4><p>Monto: $${prestamo.monto}</p><p>Deuda Actual: <strong class="text-red">$${deuda.toFixed(2)}</strong></p></div>`;
            amortTable.classList.remove('hidden');
            generarHTMLAmortizacion(prestamo, 'tabla-amortizacion-admin');
        } else {
            infoDiv.innerHTML = `<p class="text-gray" style="text-align:center;">Sin deudas activas.</p>`;
...
window.eliminarUsuario = async (u) => { if(confirm('¿Eliminar usuario?')) { await deleteDoc(doc(db, "usuarios", u)); cargarUsuariosAdmin(); } };
...
window.cargarTransaccionesGlobales = async () => {
    ...
    const snapshot = await getDocs(collection(db, "usuarios"));
    let transacciones = [];
    snapshot.forEach(doc => { const u = doc.data(); if(u.movimientos) u.movimientos.forEach(m => transacciones.push({ ...m, usuario: u.username })); });
    transacciones = transacciones.slice(-20).reverse();
    tbody.innerHTML = '';
    transacciones.forEach(t => { tbody.innerHTML += `<tr><td>${t.fecha}</td><td>${t.usuario}</td><td>${t.desc}</td><td style="color:${t.monto>0?'lightgreen':'red'}">$${Math.abs(t.monto).toFixed(2)}</td></tr>`; });
};
