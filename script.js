...
let clientePrestamoCajero = null; // Para pagos de préstamo en cajero
let rolSeleccionadoRegistro = 'cliente'; 
const defaultAvatar = "https://cdn-icons-png.flaticon.com/512/149/149071.png";
let chartInstance = null, barInstance = null;
...
function actualizarUI() {
    ...
    (usuario.movimientos||[]).slice().reverse().slice(0,10).forEach(m=>{
        list.innerHTML+=`<li><div><strong>${m.desc}</strong><br><small>${m.fecha}</small></div><div class="${m.monto>0?'text-green':'text-red'}">$${Math.abs(m.monto).toFixed(2)}</div></li>`;
    });
    ...
    // TABLA AMORTIZACIÓN (GENERACIÓN AUTOMÁTICA)
...
// NUEVA FUNCIÓN: PAGO DE PRÉSTAMO (USUARIO) - DEBITADO
window.pagarPrestamoUser = async () => {
    ...
    if(monto > p.saldoPendiente) return toast('El monto excede la deuda','error');
    
    // LOGICA DE PAGO
    usuario.saldo -= monto; // Se debita
    p.saldoPendiente -= monto; // Se reduce deuda
    usuario.movimientos.push({desc:'Pago de Préstamo (Online)', monto: -monto, fecha: new Date().toLocaleDateString()});
...
// NUEVO: CAJERO PAGO PRÉSTAMO (DINERO FÍSICO)
window.buscarClientePrestamo = async () => {
    ...
    document.getElementById('cajero-loan-amount').innerText = `$${prestamo.saldoPendiente.toFixed(2)}`;
};
...
window.pagarPrestamoCajero = async () => {
    ...
    if(monto<=0 || monto>prestamo.saldoPendiente) return toast('Monto inválido','error');
    
    // CAJERO: Solo reduce deuda, NO SALDO
    prestamo.saldoPendiente -= monto;
    clientePrestamoCajero.movimientos.push({desc:'Pago Préstamo (Efectivo)', monto:0, fecha:new Date().toLocaleDateString()}); 
...
// NUEVO: VER DETALLE CLIENTE (ADMIN)
...
function generarHTMLAmortizacion(solicitud, elementId) {
    ...
    tbody.innerHTML = '';
    // Cálculo: cuota fija = total / meses
    ...
    for(let i=1; i<=solicitud.plazo; i++) {
        ...
        tbody.innerHTML += `<tr>
            <td>Mes ${i}</td>
            <td class="text-gold">$${cuota.toFixed(2)}</td>
        ...
        (u.solicitudes||[]).forEach(sol=>{
            if(sol.estado==='pendiente'){
                c.innerHTML+=`<div class="request-card">
                    ...
                    <p>Total a Pagar: <strong>$${sol.totalPagar.toFixed(2)}</strong></p>
...
window.cargarDashboardAdmin = async () => {
    ...
    barInstance = new Chart(document.getElementById('graficoBarras'), { type:'bar', data:{labels:['Capital','Meta'], datasets:[{label:'USD',data:[cap,10000], backgroundColor:['#e6b333','#3498db']}]} });
};
